# Changelog — GEOX MCP Apps / A-FORGE
...
- **A-FORGE AgentZeroTool**: Corrected API endpoint path from `/api_message` to `/api/api_message` in all three tool methods (Delegate, Browser, Document)
- **A-FORGE LongTermMemory**: Added warning log on Qdrant federation upsert failure — previously failed silently
- **A-FORGE docker-compose**: Added missing `AGENT_ZERO_API_KEY` environment variable
