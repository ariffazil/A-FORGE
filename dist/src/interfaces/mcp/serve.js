...
function evaluatePolicyGate(toolName, toolArgs, actorId, clientIp, transport) {
    try {
...
export async function startMcpServer(transportType, port) {
        ...
        const httpServer = createServer(async (req, res) => {
            ...
            if (req.url === "/mcp") {
                ...
                if (req.method === "POST") {
                    ...
                    const rawAcceptIdx = req.rawHeaders.findIndex((h) => h.toLowerCase() === "accept");
                    if (rawAcceptIdx >= 0) {
                        let patched = req.rawHeaders[rawAcceptIdx + 1];
                        if (!patched.includes("application/json"))
                            patched += ", application/json";
                        if (!patched.includes("text/event-stream"))
                            patched += ", text/event-stream";
                        ...
                        const tools = getServerTools().map(t => ({
                            ...
                            inputSchema: {
                                ...
                                properties: t.inputSchema?.shape
                                    ? Object.entries(t.inputSchema.shape).reduce((acc, [k, v]) => {
                    ...
                    if (method === "tools/call") {
                        ...
                        if (!aThinkVerdict.allowed) {
                            ...
                            process.stderr.write(`[A-FORGE-MCP] A-THINK ${aThinkVerdict.status} (stateless): ${toolName} — ${aThinkVerdict.reason}\n`);
                            res.writeHead(403, {
                                "Content-Type": "application/json",
                        ...
                        const actorHint = toolArgs?.actor_id || toolArgs?.actorId || "stateless-client";
                        const clientIp = (req.headers["x-forwarded-for"]?.toString().split(",")[0]?.trim()
                            || req.socket?.remoteAddress || "unknown");
                        const policyVerdict = evaluatePolicyGate(toolName, toolArgs, actorHint, clientIp, "http");
                        if (policyVerdict.verdict === "DENY") {
                            process.stderr.write(`[A-FORGE-MCP] Policy DENY (stateless) actor=${actorHint} tool=${toolName} reasons=${policyVerdict.reasons.join(",")}\n`);
                            res.writeHead(403, {
                                "Content-Type": "application/json",
                                "X-Policy-Gate": "DENY",
                    ...
                    if (method === "prompts/list") {
                        const registry = server._registeredPrompts;
                        const promptsList = registry ? Object.entries(registry)
                            .filter(([_, p]) => p.enabled !== false)
                            .map(([name, p]) => ({
                            name,
                            description: p.description ?? "",
                            arguments: p.argsSchema ? Object.entries(p.argsSchema.shape).map(([k, v]) => ({
                                name: k,
                    ...
                    if (method === "prompts/get") {
                        ...
                        if (typeof prompt.callback === "function") {
                            ...
                            const argDefs = prompt.argsSchema ? Object.entries(prompt.argsSchema.shape).map(([k, v]) => ({
                                name: k,
                                description: v.description,
                                required: !v._def?.typeName?.includes("optional"),
                            })) : [];
...
if (isMainModule) {
    const transport = (process.argv.find(a => a.startsWith("--transport="))?.split("=")[1] || "http");
