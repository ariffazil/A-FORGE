...
import { readFeatureFlags } from "../flags/featureFlags.js";
...
export function readRuntimeConfig() {
    const trustLocalVps = process.env.AGENT_WORKBENCH_TRUST_LOCAL_VPS === "1" ||
    ...
    return {
        ...
        apiPricing: {
            inputCostPerMillionTokens: Number(process.env.AGENT_WORKBENCH_INPUT_COST_PER_MILLION_TOKENS ?? "0"),
            outputCostPerMillionTokens: Number(process.env.AGENT_WORKBENCH_OUTPUT_COST_PER_MILLION_TOKENS ?? "0"),
        },
        memoryPath: process.env.AGENT_WORKBENCH_MEMORY_PATH ??
            resolve(homedir(), ".agent-workbench", "memory.json"),
        scoreboardPath: process.env.AGENT_WORKBENCH_SCOREBOARD_PATH ??
            resolve(homedir(), ".agent-workbench", "scoreboard.json"),
        runMetricsDir: process.env.AGENT_WORKBENCH_RUN_METRICS_DIR ??
            resolve(homedir(), ".agent-workbench", "metrics"),
        trustLocalVps,
        defaultMode: process.env.AGENT_WORKBENCH_DEFAULT_MODE === "external"
            ? "external_safe_mode"
            : trustLocalVps
                ? "internal_mode"
                : "external_safe_mode",
