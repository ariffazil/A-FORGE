 ...
 *
 * Tests verify that:
 *   - registerVerifiedSession with valid SCT → verified_session authority
 ...
 *   - registerVerifiedSession without SCT is a no-op (P0.5 fail-closed)
 */

import { describe, it } from "node:test";
import { strict as assert } from "node:assert";
import { McpPolicyGate, buildSCT } from "../src/domain/governance/McpPolicyGate.js";
...
describe("P0.1 — verifiedSessions map is the canonical verified-session path", () => {
  ...
  it("registerVerifiedSession with valid SCT + session_id grants verified_session per-request", () => {
    const gate = new McpPolicyGate();
    const sct = buildSCT("arif", "sess-1", SECRET);
  ...
  it("Multiple sessions coexist with different actor_ids", () => {
    const gate = new McpPolicyGate();
    const sct1 = buildSCT("arif", "sess-arif", SECRET);
    const sct2 = buildSCT("forge-worker", "sess-worker", SECRET);
  ...
  it("Wrong session_id does NOT pick up another session's actor", () => {
    const gate = new McpPolicyGate();
    const sct = buildSCT("arif", "sess-arif", SECRET);
  ...
  it("registerVerifiedSession without SCT is a no-op (P0.5 fail-closed)", () => {
    const gate = new McpPolicyGate();
    const ok = gate.registerVerifiedSession("sess-1", "arif");
    assert.equal(ok, false, "No SCT → registration rejected");
