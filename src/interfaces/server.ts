...
export function createApp(): express.Express {
  ...
  mcpRouter.use("/mcp", (req: Request, _res: Response, next: NextFunction) => {
    // Session ID injection (P0-3 — 2026-06-19)
  ...
  const mcpHandler = async (req: Request, res: Response) => {
    ...
    // P0-3 fix (2026-06-19): MCP SDK 1.9.0 StreamableHTTPServerTransport
    // requires Mcp-Session-Id header on every POST. Inject the known
    // session ID generated by sessionIdGenerator at transport init.
    // (We set this here rather than in middleware because the SDK
    // may snapshot headers before middleware can mutate them.)
    if (req.method === "POST" && !req.headers["mcp-session-id"]) {
      req.headers["mcp-session-id"] = "a-forge-gateway-session";
    }
    try {
...
async function initMcpTransport(): Promise<StreamableHTTPServerTransport | null> {
  try {
    ...
    const transport = new StreamableHTTPServerTransport({
      // P0-3 fix (2026-06-19): Fixed session ID. The mcpHandler injects
      // "a-forge-gateway-session" into Mcp-Session-Id header on every POST
      // before the transport sees the request. This sidesteps the SDK 1.9.0
      // requirement that clients send the header on first contact.
