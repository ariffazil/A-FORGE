...
- **Tool surface:** 111 MCP tools (count from live `tools/list` on :7072; 2026-07-19 verified)
...
- **A-FORGE MCP (7072 + stdio)** = Hands / Actuator / Forger: owns execution (111 tools), leases, proxies, build/deploy/run/shell/browser. **Never** issues constitutional verdicts or self-authorizes.
...
All 111 forge_* tools treat every input as literal. A joke is not a command. A laugh is not a confirmation. "Flush everything lah" ≠ permission to reset state.
